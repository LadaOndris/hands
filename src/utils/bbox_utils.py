...
def generate_prior_boxes(feature_map_shapes, aspect_ratios):
    ...
    for i, feature_map_shape in enumerate(feature_map_shapes):
        wh_pairs = get_wh_pairs(aspect_ratios[i], len(feature_map_shapes) - i, len(feature_map_shapes))
...
def get_scale_for_nth_feature_map(k, m=4, scale_min=0.10, scale_max=0.75):
...
def get_deltas_from_bboxes(prior_boxes, bboxes):
    """Calculating bounding box deltas for given ground truth boxes.
    inputs:
        prior_boxes = (total_bboxes, [center_x, center_y, width, height])
        bboxes = (batch_size, total_bboxes, [y1, x1, y2, x2])
    outputs:
        deltas = (batch_size, total_bboxes, [delta_bbox_y, delta_bbox_x, delta_bbox_h, delta_bbox_w])
    """
    gt_width = bboxes[..., 3] - bboxes[..., 1]
    gt_height = bboxes[..., 2] - bboxes[..., 0]
    gt_ctr_x = bboxes[..., 1] + 0.5 * gt_width
    gt_ctr_y = bboxes[..., 0] + 0.5 * gt_height
    ...
    return tf.stack([delta_y, delta_x, delta_h, delta_w], -1)
...
def get_weighted_boxes(scores, bboxes, mask):
    ...
    inputs:
        ...
        mask = (total_bboxes,)
    outputs:
        weighted_bbox = (1, [y1, x1, y2, x2])
    """
    selected_scores = scores[mask]
    selected_bboxes = bboxes[mask]
    weighted_sum = tf.reduce_sum(selected_bboxes * selected_scores, 0)
...
def weighted_suppression_body(counter, iou_threshold, scores, bboxes_and_landmarks, weighted_suppressed_data):
    ...
    weighted_bbox_and_landmark = get_weighted_boxes(scores, bboxes_and_landmarks, overlapped_mask)
...
def weighted_suppression(scores, bboxes_and_landmarks, max_total_size=50, score_threshold=0.75, iou_threshold=0.3):
    ...
    inputs:
        scores = (total_bboxes, [probability])
        bboxes_and_landmarks = (total_bboxes, [y1, x1, y2, x2])
    ...
    cond = lambda counter, iou_threshold, scores, data, weighted: tf.logical_and(tf.less(counter, max_total_size),
                                                                                 tf.greater(tf.shape(scores)[0], 0))
    _, _, _, _, weighted_data = tf.while_loop(cond, weighted_suppression_body,
                                              [counter, iou_threshold, sorted_scores, sorted_bboxes_and_landmarks,
                                               weighted_data])
    #
