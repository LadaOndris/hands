...
def square_bboxes(bboxes, image_size):
...
def augment_batch(images, uv_joints):
    images = tf.cast(images, dtype=tf.float32)
    im_height, im_width = images.shape[1], images.shape[2]
    image_center = [im_width / 2, im_height / 2]
    images, joints = rotate(images, uv_joints, image_center)
    return images, joints


def find_largest_countour(image):
    image_tensor = image.to_tensor()
    image_casted_uint16 = tf.cast(image_tensor, dtype=tf.uint16)

    def _find_largest_countour(image):
        image = np.ascontiguousarray(image)
        # Use morphological closing to connect disconnected fingers
        # caused by a depth camera
        image_morph_closed = cv2.morphologyEx(image, cv2.MORPH_CLOSE,
    ...
    cleared_img = tf.py_function(_find_largest_countour, inp=[image_casted_uint16],
                                 Tout=image_tensor.dtype)

    return tf.RaggedTensor.from_tensor(cleared_img, ragged_rank=2)


def clear_hand(images):
    cleared = tf.map_fn(find_largest_countour, elems=images,
                        fn_output_signature=tf.RaggedTensorSpec(shape=[None, None, 1], dtype=images.dtype))
    return cleared


def bcubes_translate3d(bcubes, stddev):
    """
...
def bcubes_scale3d(bcubes, stddev, cube_size):
    """
    ...
    half_cube_size = tf.constant(cube_size, dtype=tf.float32) / 2
...
def normalize_coords(joints_uv, joints_z, bcubes, image_size):
    ...
    normalized_uv = joints_uv / image_size
...
def compute_offsets(images, joints, image_out_size):
    """
    ...
    z_im = resize_bilinear_nearest_batch(images, [image_out_size, image_out_size])
...
def convert_coords_to_global(local_uvz_coords, bboxes):
...
class DatasetPreprocessor:
    ...
    @timing
    def postprocess(self, y_pred, bboxes, bcubes, resize_coeffs):
        """
        ...
        global_z = normalized_z * self.cube_size[2] + bcubes[..., tf.newaxis, 2:3]
        uv_local = 1.0 / resize_coeffs[:, tf.newaxis, :] * resized_uv
        uv_global = uv_local + tf.cast(bboxes[:, tf.newaxis, :2], dtype=tf.float32)
    ...
    def preprocess(self, images, bboxes):
        ...
        # cropped_imgs = self.clear_hand(self.cropped_imgs)  # find countours
        bcubes = tf.cast(bcubes, tf.float32)
        bboxes = tf.concat([bcubes[..., 0:2], bcubes[..., 3:5]], axis=-1)
