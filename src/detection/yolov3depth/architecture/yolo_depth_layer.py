...
class YoloDepthLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs_and_image):
        """
        ...
        tf.print(tf.shape(self.anchors), self.anchors)  # [1 2] ListWrapper([(5.0, 5.0)])
        tf.print(tf.shape(image))                       # [4 416 416 1]
